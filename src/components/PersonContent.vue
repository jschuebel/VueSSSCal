...
//                    var hdr = response.Headers.GetValues("Paging-TotalRecords").FirstOrDefault();
    ...
    var filterParams = `page=${currentPage}&pageSize=${pageSize}&sort[0][field]=name&sort[0][dir]=asc`;

//			  fetch(`http://localhost:5010/api/person?&${filterParams}`)
			  fetch(`http://www.schuebelsoftware.com/SSSCalCoreApi/api/person?${filterParams}`)
				.then(response => {
